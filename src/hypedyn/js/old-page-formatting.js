// old page formatting, no longer used
...
function htmlFormat(content, links, noformat) {
	...
	function plain_text_code2 ( offset, linktext, link ) {
		...
		replace_info = escape_special( plaintext );
		...
		if (alt_offset == -1) { // not breaking link text
			...
			if ( space_pos == -1 && nlpos == -1 && next_link_pos == -1 ) {
				...
				offset_inc = plaintext.length - replace_offset;
			} else if ( space_pos == 0 ) {
			...
			if ( space_pos == -1 && nlpos == -1 ) {
				...
				alt_offset += plaintext.length - replace_offset;
			} else if ( space_pos == 0 ) {
				alt_offset++;
				newcode = start_tag + " " + end_tag;
			} else if ( nlpos == 0 ) {
